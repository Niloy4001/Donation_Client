...
const EventList = () => {
  ...
  useEffect(() => {
    try {
      const res = axiosPublic.get("/events").then((res) => setEvents(res.data));
    } catch (error) {
      toast.error(error);
    }
  }, [events]);
      ...
      {events.length > 0 && (
        <>
          <div className=" mb-8 max-w-2xl">
            <h1 className="text-4xl font-bold text-gray-800 flex items-center gap-1.5">
            ...
            <p className="text-gray-600 mt-2">
              See All events and Manage your way
            </p>
          </div>
                ...
                {events?.map((item) => (
                  <tr key={item._id}>
                    ...
                    <td className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => handleDetlete(item._id)}
